...
function process_coin($symbol, $name, $project_url, $branch) {
    ...
    process_amounts($data, $raw_urlbase, $symbol);
...
function process_chainparams(&$data, $urlbase, $meta) {
    ...
    if(!$buf) {
        warn("chainparams.cpp not found. Perhaps coin is too old or wrong branch?");
        return false;
    }
    ...
    foreach($chunks as $chunk) {
        if(strstr($chunk, '   CMainParams()')) {
            $main = $chunk;
        }
        else if(strstr($chunk, '   CTestNetParams()')) {
            $testnet = $chunk;
        }
        else if(strstr($chunk, '   CRegTestParams()')) {
            $regtest = $chunk;
        }
    }
...
function process_amounts(&$data, $urlbase, $symbol) {
